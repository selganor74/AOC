...
const answerCounter = (questionAnsweredYes = "", sumToThisObject = {}) => {
    for(let i = 0; i < questionAnsweredYes.length; i++) {
        const question = questionAnsweredYes.charAt(i);
        sumToThisObject[question] = ( sumToThisObject[question] || 0 ) + 1; 
    }
}

const groupAnyoneYesCounter = (groupsAnswers = []) => {
    let questions = {};
    for(let answers of groupsAnswers) {
        answerCounter(answers, questions)
    }
    let toReturn = 0;
    for(let q in questions) {
        // console.log(`question ${q} had ${questions[q]} answers.`)
        toReturn++;
    }
    return toReturn;
}

const groupEveryoneYesCounter = (groupsAnswers = []) => {
    let questions = {};
    for(let answers of groupsAnswers) {
        answerCounter(answers, questions)
    }
    let toReturn = 0;
    for(let q in questions) {
        if (questions[q] == groupsAnswers.length)
            toReturn++;
    }
    return toReturn;
}

const counter = (algorithm) => {
    let group = [];
    let count = 0;
    ...
    for(let i = 0; i < input.length; i++) {
        let currAns = input[i];
        if(currAns == "") {
            // console.log("group detected", group);
            // we have a group complete
            count = count + algorithm(group);
...
const part1 = () => {
    ...
    count = counter(groupAnyoneYesCounter);

    console.log(`... and the answer to part 1 is ${count}`);
}

const part2 = () => {
    
    count = counter(groupEveryoneYesCounter);

    console.log(`... and the answer to part 2 is ${count}`);
}
