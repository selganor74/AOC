...
const isValid = (row) => {
    ...
    row.split(" ").forEach( t => {
        ...
        let val;
        [prop, val] = t.split(":");
        passportData[prop] = val;
    });
    let counter = 0;
    properties.forEach(p => passportData[p] ? counter++ : 0);
    if(counter == properties.length) return true;
    if(counter == properties.length - 1 && !passportData["cid"]) return true;
    return false;
}
