...
const isValid = (row) => {
    ...
    const validators = {
        ...
        "hgt": (val) => {
            if (!val) return false;
            const uom = val.substring(val.length - 2);
            const hs = val.substring(0, val.length - 2);
            const hn = Number.parseInt(hs);
            switch (uom) {
                case "in":
                    return hn >= 59 && hn <= 76;
                case "cm":
                    return hn >= 150 && hn <= 193;
                default:
                    return false;
            }
        },
    ...
    row.split(" ").forEach(t => {
        ...
        let val;
        [prop, val] = t.split(":");
        passportData[prop] = val.trim();
    });
    let isValid = true;
    properties.forEach(p => isValid = isValid && validators[p](passportData[p]));
    if (isValid) console.log(row);
    return isValid;
}


const runPassportValidator = () => {
    let tempRow = "";
    let valids = 0;
    for (let row of input) {
        ...
        if (row === "") {
            valids += isValid(tempRow) ? 1 : 0;
            tempRow = ""
        }
    }
    console.log(`... and the answer is ${valids} valid passports`);
}
