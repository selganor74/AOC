...
const ruleParser = (rule = "") => {
    let bagName;
    let restOfRule;
    [ bagName, restOfRule ] = rule.split(" bags contain ");
    let canContain = restOfRule.split(", ");
    var newBag = allBags[bagName] || new Bag();
    newBag.name = bagName;
    allBags[bagName] = newBag;

    for(let contained of canContain) {
        [numContained, pt1, pt2, bag] = contained.split(" ");
        if (!pt1) console.log(contained, restOfRule);
        let containedBagName = pt1 + " " + pt2;
        let containedBag = allBags[containedBagName] || new Bag();
        containedBag.name = containedBagName;
        newBag.contains[containedBagName] = { 
            numContained: Number.parseInt(numContained == "no" ? "0" : numContained), // numContained can contain "no" of "no other bags" 
            bag: containedBag
        };
        allBags[containedBagName] = containedBag;
    }
}
...
const containsColor = (bag = new Bag(), color = "") => {
    if (bag.contains[color]) 
        return true;
    
    for(let innerBagColor in bag.contains) {
        if( containsColor(bag.contains[innerBagColor].bag, color) ) return true;
    }
    return false;
}

const countContainedBags = (bag = new Bag() ) => {
    let directly = 0; 
    let indirectly = 0;
    let bagContent = bag.contains;
    // console.log(`${bag.name} : counting bags for`)
    for(let innerBagColor in bagContent) {
        // console.log(`${bag.name} : ${bagContent[innerBagColor].numContained} x ${innerBagColor}`);
        directly += bagContent[innerBagColor].numContained;
        indirectly += bagContent[innerBagColor].numContained * countContainedBags(bag.contains[innerBagColor].bag);
    }
    return directly + indirectly;
}

const part1 = () => {
    // find how many colors can contain a shiny gold
    let counter = 0;
    for(let color in allBags) {
        if(containsColor(allBags[color], "shiny gold"))
            counter++;
    }
    console.log(`... and the answer is ${counter} bags can directly or indirectly contain a shiny gold one.`);
}

const part2 = () => {
    let bag = allBags["shiny gold"];
    let bagsCount = countContainedBags(bag);
    console.log(`... and the answer is ${bagsCount} bags can be contained in a shiny gold one.`);
}
